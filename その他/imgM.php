...
define('MOE_IPC', '1');				// 連続投稿にIP規制をかけるか否か  y:1 n:0  Ver 2.07
        ...
        if(is_file($logmoe)){
          ...
          if($resno){ // 投票表示用
            ...
            if($mcountlog == 'DEN'){
              ...
              if(MOE_BOT){
                $votext.="<font color=#cc1105 size=+1></font><input type=image src=".MOE_BOTP.">";
              }else{
                $votext.="<font color=#cc1105 size=+1></font><input type=submit class=moetype2 value=".MOE_BOTT2.">";
              }
              if ($mcountlog >= MOE_DCNT*0.8){
...
function adminmoe($pass){
  ...
  echo "<center><LI>ラストカウントIPにカーソルを乗せると全カウントIPログが表示されます。\n";
  ...
  for($j = 0; $j < count($line); $j++){
    ...
    if($ext && is_file($path.$time.$ext)){
      ...
      for($m=0; $m<count($mp_data); $m++){
        ...
        $mcountip2= trim($mcountip);
        $datip.="[$mcountlog]$mcountip2";
      }
      ...
      echo "<td><a title=$datip>$mcountip</a></td><td align=center>$clip($size)</td>".
...
function moecount(){
  ...
  for($m=0; $m<count($logmoe); $m++){
    list($vcountlog) = split(",",$logmoe[$m]);
  }

  if(MOE_IPC){
    for($m=0; $m<count($logmoe); $m++){
      list($cct,$vipck) = split(",",$logmoe[$m]);
      if(trim($vipck)==$ip){
        $ipflag++;
      }
    }
  ...
  if($ipflag){
    echo "<b><big>ひとつの画像に複数回投票はできません。</big></b>";
  }else{
    echo "<b><big>萌えカウントに投票しました。</big></b>";
  }
  echo '<BR><BR>--- MoeCountSystem Ver 2.07 ---</P>';
  updatelog();
