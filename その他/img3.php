...
define('AD_INSERT', '0');			// 広告を挿入する  する:1  しない:0

//レス画像添付機能-------------------------------------------------------------
define('RES_IMG', '1');				// レスにも画像を添付できるようにする  添付可能:1 添付不可:0
define('RES_IMG_LIMIT', '100');		// レス画像上限枚数
...
/* 投稿フォーム */
function form(&$dat,$resno,$admin="",$imageflag){
  global $addinfo;
  $maxbyte = MAX_KB * 1024;
  $no=$resno;
  ...
  if(RES_IMG && $imageflag && $resno){
    $dat.='<font color="red"><b> 画像付レス可</b></font>';
  }
...
//          }
      ...
      // 画像レス数告知
      if(RES_IMG && RES_IMG_LIMIT_NOTICE && $resno){
