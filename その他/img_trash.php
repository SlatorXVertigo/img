...
//負荷軽減html経由関係---------------------------------------------------------
define('RES_FILE', '1');			// レスをhtml経由にする  する:1 しない:0
...
define('DEN_MSG2', '隊長！殿堂入りです！');// 殿堂入りメッセージ（画像付レスの場合
...
function updatelog($resno=0){
  ...
  for($page=0;$page<$counttree;$page+=PAGE_DEF){
    ...
    if(!$resno){ $st = $page; }
    if(!$resno || !MOE_COUNT){ $dat.='<form action="'.$self_path.'" method=POST>'."\n"; }
    ...
    for($i = $st; $i < $st+PAGE_DEF; $i++){
      ...
      if(is_file($logmoe)){
        ...
        if($resno){ // 投票表示用
          ...
          if($mcountlog == 'DEN'){
            $votext.="<center><b><font color=#ff0000 size=+2>".DEN_MSG."</font></b></center>";
          }else{
            $votext.="\n<b><center>".
            ...
            if(MOE_BOT){
              $votext.="<font color=#cc1105 size=+1>投票：</font><input type=image src=".MOE_BOTP.">";
            }else{
              $votext.="<font color=#cc1105 size=+1>投票：</font><input type=submit class=moetype value=".MOE_BOTT.">";
            }
            if($mcountlog >= MOE_DCNT*0.8){
              $votext.="　現在:<font color=#cc1105 size=+5>$mcountlog</font>".MOE_MSG5."</center></form></b>";
            }elseif ($mcountlog >= MOE_DCNT*0.6){
              $votext.="　現在:<font color=#cc1105 size=+4>$mcountlog</font>".MOE_MSG4."</center></form></b>";
            }elseif ($mcountlog >= MOE_DCNT*0.4){
              $votext.="　現在:<font color=#cc1105 size=+3>$mcountlog</font>".MOE_MSG3."</center></form></b>";
            }elseif ($mcountlog >= MOE_DCNT*0.2){
              $votext.="　現在:<font color=#cc1105 size=+2>$mcountlog</font>".MOE_MSG2."</center></form></b>";
            }elseif ($mcountlog >= 1){
              $votext.="　現在:<font color=#cc1105 size=+1>$mcountlog</font>".MOE_MSG1."</center></form></b>";
            } else {
              $votext.="　現在:<font color=#117783 size=+1>0</font>".MOE_MSG0."</center></form></b>";
            }
          }
        }
        // カウント表示用
...
//          }
      ...
      if(RES_FILE){
        if(!$resno) $dat.="[<a href=\"".RES_DIR.$time.".htm\">返信+投票</a>]";
      }else{
        if(!$resno) $dat.="[<a href=".$self_path."?res=$no>返信+投票</a>]";
      }
      $dat.="<BR>\n";
      ...
      for($k = $s; $k < count($treeline); $k++){
        ...
        if(is_file($logmoe)){
          ...
          if($resno){ // 投票表示用
            ...
            if($mcountlog == 'DEN'){
              $votext.="<b><font color=#ff0000 size=+2>".DEN_MSG2."</font></b>";
            }else{
              $votext.="\n<b>".
                       "<form action=".$self_path." method=POST>".
                       "<input type=hidden name=moeno value=$no>".
              ...
              if(MOE_BOT){
                $votext.="<input type=image src=".MOE_BOTP.">";
              }else{
                $votext.="<input type=submit class=moetype2 value=".MOE_BOTT2.">";
              }
              if ($mcountlog >= MOE_DCNT*0.8){
                $votext.="　<font color=#cc1105 size=+5>$mcountlog</font>".MOE_MSG5."</form></b>";
              }elseif ($mcountlog >= MOE_DCNT*0.6){
                $votext.="　<font color=#cc1105 size=+4>$mcountlog</font>".MOE_MSG4."</form></b>";
              }elseif ($mcountlog >= MOE_DCNT*0.4){
                $votext.="　<font color=#cc1105 size=+3>$mcountlog</font>".MOE_MSG3."</form></b>";
              }elseif ($mcountlog >= MOE_DCNT*0.2){
                $votext.="　<font color=#cc1105 size=+2>$mcountlog</font>".MOE_MSG2."</form></b>";
              }elseif ($mcountlog >= 1){
                $votext.="　<font color=#cc1105 size=+1>$mcountlog</font>".MOE_MSG1."</form></b>";
              } else {
                $votext.="　<font color=#117783 size=+1>0</font>".MOE_MSG0."</form></b>";
              }
            }
            // $datに萌えテキストをくっつける
            // $dat .= $votext;
...
function valid($pass){
  ...
  echo "[<a href=\"".PHP_SELF."?allview=img\">全表示</a>]\n";
...
function imgallview(){
  ...
  echo '<HTML>'."\n".'<HEAD>'."\n".
       ...
       '</HEAD>'."\n".
       '<body bgcolor="'.BG_COL.'" text="'.TXT_COL.'" link="'.LINK_COL.'" vlink="'.VLINK_COL.'">'."\n".
       '<div align=right>[<a href='.PHP_SELF2.'>掲示板に戻る</a>] [<a href="'.PHP_SELF.'?mode=admin">管理用</a>]</div>';

  echo "<center><LI>全画像データー表示システム（実験中。。。）\n";
  echo "<table border=1 cellspacing=0>\n";
  echo "<tr bgcolor=6080f6><th>記事 No.</th><th>投稿日</th><th>題名</th>";
  echo "<th>投稿者</th><th>コメント</th><th>添付<br>(Bytes)</th>";
  echo "<th>ﾒｰﾀｰ<BR><img src=".MOE_BOTP."></th><th>ｶｳﾝﾄ</th>";
  echo "</tr>\n";
  $line = file(LOGFILE);
  $bgcol = 0;
  ...
  for($j = 0; $j < count($line); $j++){
    ...
    if($ext && is_file($path.$time.$ext)){
      ...
      if(strlen($com) > 20) $com = substr($com,0,18) . ".";
      if($no){
        if(RES_FILE && is_file(RES_DIR.$time.".htm")){
          $no = "<a href=\"".RES_DIR.$time.".htm\" target=_blank>$no</a>";
        }
        if(!RES_FILE){
          $no = "<a href=".PHP_SELF."?res=$no target=_blank>$no</a>";
        }
      }
      // 画像があるときはリンク
      if($ext && is_file($path.$time.$ext)){
        $img_flag = TRUE;
        if(IMG_REFER){
          $clip = "<a href=\"".IMG_REF_DIR.$time.".htm\" target=_blank>".$time.$ext."</a><br>";
        }else{
          $clip = "<a href=\"".IMG_DIR.$time.$ext."\" target=_blank>".$time.$ext."</a><br>";
        }
        $size = filesize($path.$time.$ext);
      ...
      echo "<th>$no</th><td><small>$now</small></td><td>$sub</td>";
      echo "<td><b>$name</b></td><td><small>$com</small></td>";
      echo "<td align=center>$clip($size)</td>".
           "<td><img src=".MOE_BOTP." width=$bar % height=20></td>".
           "<th>$mcountlog</th>\n";
      echo "</tr>";
    }
